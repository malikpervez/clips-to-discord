...
{
    private static readonly StableVersion InstalledVersion = new(1, 3, 5);
    ...
    {
        Assert(StableVersion.TryParse("v1.3.5", out var parsedVersion) && parsedVersion == InstalledVersion,
            "Stable versions must accept the repository's v-prefixed release tags.");
        Assert(!StableVersion.TryParse("v1.3.5-beta.1", out _),
        ...
        await AssertStatusAsync(BuildReleaseJson(tag: "v1.3.4"), UpdateCheckStatus.UpToDate,
